...
import './GridLayoutTestPage.css';

// Grid layout types
type GridLayoutType = 'masonry' | 'uniform' | 'staggered' | 'waterfall' | 'cards' | 'magazine';
...
// Create a shared image pool with stable URLs for caching
// Use fixed seeds so URLs don't change - this allows browser caching
const allImagesPool: ImageItem[] = [
  ...
  ...Array.from({ length: 20 }, (_, i) => ({
    ...
    thumbnailUrl: `https://picsum.photos/seed/arch${i}/60/80`,
    width: 600,
    height: 800,
    aspectRatio: 600 / 800,
  ...
  ...Array.from({ length: 20 }, (_, i) => ({
    ...
    thumbnailUrl: `https://picsum.photos/seed/nature${i}/70/70`,
    width: 700,
    height: 700,
    aspectRatio: 1,
    title: `Nature ${i + 1}`,
...
// Map category names to URL-friendly slugs (like Unsplash)
...
function getCategoryImages(category: string): ImageItem[] {
  ...
  // Note: Browser will cache these images based on URL
  // When you switch back to a category, images should load from cache
  return allImagesPool.filter(img => img.category === category);
}

// Global store for images per category (persists across route changes - Unsplash technique)
const globalImagesCache = new Map<string, ImageItem[]>();

// Track which images are preloaded and ready to display (Unsplash technique)
const preloadedImages = new Map<string, Set<string>>(); // category -> Set of image IDs

/**
 * Preload images and wait for them to be ready (Unsplash technique)
...
export default function GridLayoutTestPage() {
  ...
  // Store images per category - persists in global cache (Unsplash technique)
  ...
  const [gridLayout, setGridLayout] = useState<GridLayoutType>('masonry');
  const [isLoading, setIsLoading] = useState(false);
  
  // Get display images from cache or load them
  const getDisplayImages = (category: string): ImageItem[] => {
    // Check global cache first (Unsplash technique)
  ...
  const [newImagesReady, setNewImagesReady] = useState(false);
  const [cachedImageIds, setCachedImageIds] = useState<Set<string>>(new Set()); // Track cached images
  const [loadedImageIds, setLoadedImageIds] = useState<Set<string>>(new Set()); // Track which images are fully loaded (Unsplash technique)
  ...
  useEffect(() => {
    if (!categorySlug) {
      ...
      navigate('/test-grid-layouts/all', { replace: true });
      return;
    }
  }, [categorySlug, navigate]);
  
  // Sync with URL when category changes (Unsplash technique)
  ...
  useLayoutEffect(() => {
    ...
    // Check global cache first (Unsplash technique - this is the key!)
    ...
    if (existingImages && existingImages.length > 0) {
      ...
      setNewImagesReady(false); // Start hidden, will show as they load
      setIsLoading(false);
      ...
      // Store in global cache immediately (Unsplash technique)
  ...
  // Handle category change from UI (navigates to route - Unsplash technique)
  const handleCategoryChange = (category: string) => {
    if (isLoading) return;
    
    // Navigate to the category route (this will trigger the useEffect above)
    const slug = categoryToSlug[category] || 'all';
    navigate(`/test-grid-layouts/${slug}`, { replace: true });
  };

  const gridLayouts: { value: GridLayoutType; label: string; description: string }[] = [
    { value: 'masonry', label: 'Masonry', description: 'Pinterest-style waterfall layout' },
    { value: 'uniform', label: 'Uniform Grid', description: 'Instagram-style square grid' },
    { value: 'staggered', label: 'Staggered', description: 'Alternating offset layout' },
    { value: 'waterfall', label: 'Waterfall', description: 'Vertical flowing layout' },
    { value: 'cards', label: 'Cards', description: 'Card-based grid with spacing' },
    { value: 'magazine', label: 'Magazine', description: 'Editorial-style layout' },
  ];

  return (
    <div className="grid-layout-test-page">
      <div className="grid-layout-controls">
        <div className="control-section">
          <h2>Grid Layout Styles</h2>
          <div className="layout-buttons">
            {gridLayouts.map((layout) => (
              <button
                key={layout.value}
                className={`layout-btn ${gridLayout === layout.value ? 'active' : ''}`}
      ...
      <div className={`image-grid grid-${gridLayout} ${isLoading ? 'loading' : ''}`} style={{ position: 'relative' }}>
          ...
          <>
            {prevImages.map((image) => (
              <div
                ...
                style={{
                  aspectRatio: gridLayout === 'uniform' ? '1' : `${image.width} / ${image.height}`,
                  opacity: newImagesReady ? 0 : 1,
                }}
              >
                <div className="image-wrapper">
                  <img
                    ...
                    alt={image.title}
                    style={{
                      aspectRatio: gridLayout !== 'uniform' ? `${image.width} / ${image.height}` : '1',
                    }}
                  />
                </div>
              </div>
        ...
        {/* Show new images - use last image as placeholder technique (like react-native-fast-image patch) */}
        {displayImages.map((image, index) => {
          // Calculate row span for staggered layout based on aspect ratio
          const rowSpan = gridLayout === 'staggered' 
            ? Math.max(20, Math.round(image.aspectRatio < 0.8 ? 45 : image.aspectRatio > 1.2 ? 25 : 35))
            : undefined;
            ...
            >
              <div className="image-wrapper">
                ...
                {!isLoaded && prevImageAtPosition && (
                  <img
                    key={`placeholder-${prevImageAtPosition.id}`}
